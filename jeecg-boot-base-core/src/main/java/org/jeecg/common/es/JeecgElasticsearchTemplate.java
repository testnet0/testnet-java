...
public class JeecgElasticsearchTemplate {
    ...
    public JeecgElasticsearchTemplate(@Value("${jeecg.elasticsearch.cluster-nodes}") String baseUrl
            , @Value("${jeecg.elasticsearch.userPass}") String userPass
        ...
        if (StringUtils.isNotEmpty(baseUrl)) {
            ...
            this.headers = RestUtil.getHeaderApplicationJson();
            if (Strings.isNotBlank(userPass)) {
                /**
                 * 用户名:密码
                 */
                String auth = new String(Base64.encodeBase64(userPass.getBytes(StandardCharsets.ISO_8859_1)));
    ...
    private <T> ResponseEntity<T> cat(String urlAfter, Class<T> responseType) {
        ...
        return request(url, HttpMethod.GET, null, null, null, responseType);
    }
    ...
    public boolean createIndex(String indexName) {
        ...
        */
        try {
            return put(url).getBoolean("acknowledged");
        } catch (org.springframework.web.client.HttpClientErrorException ex) {
    ...
    public boolean removeIndex(String indexName) {
        String url = this.getBaseUrl(indexName).toString();
        try {
            return delete(url).getBoolean("acknowledged");
        } catch (org.springframework.web.client.HttpClientErrorException ex) {
            if (HttpStatus.NOT_FOUND == ex.getStatusCode()) {
    ...
    public boolean saveOrUpdate(String indexName, String typeName, String dataId, JSONObject data) {
       ...
       "createIndexA2": {
        ...
        try {
            String result = put(url, data).getString("result");
    ...
    public boolean saveBatch(String indexName, String typeName, JSONArray dataList) {
        ...
        request(url, HttpMethod.PUT, headers, null, bodySb, JSONObject.class);
        return true;
    }
    ...
    public boolean delete(String indexName, String typeName, String dataId) {
        ...
        */
        try {
            return "deleted".equals(delete(url).getString("result"));
        } catch (org.springframework.web.client.HttpClientErrorException ex) {
            if (HttpStatus.NOT_FOUND == ex.getStatusCode()) {
                return false;
            } else {
    ...
    public JSONObject search(String indexName, String typeName, JSONObject queryObject) {
        ...
        JSONObject res = post(url, queryObject);
        log.debug("url:" + url + " ,return res: \n" + res.toJSONString());
        return res;
    }
     ...
     * @return { "query": query }
     */
    public JSONObject buildQuery(List<String> source, JSONObject query, long from, long size) {
    ...
    private JSONObject get(String url, JSONObject variables) {
        return get(url, null, variables);
    }

    private JSONObject get(String url, Object params) {
        return get(url, null, params);
    }

    private JSONObject get(String url, JSONObject variables, Object params) {
        return request(url, HttpMethod.GET, this.headers, variables, params, JSONObject.class).getBody();
    }

    private JSONObject put(String url) {
        return put(url, null, null);
    }

    private JSONObject put(String url, JSONObject variables) {
        return put(url, null, variables);
    }

    private JSONObject put(String url, Object params) {
        return put(url, null, params);
    }

    private JSONObject put(String url, JSONObject variables, Object params) {
        return request(url, HttpMethod.PUT, this.headers, variables, params, JSONObject.class).getBody();
    }

    private JSONObject delete(String url) {
        return delete(url, (JSONObject) null, null);
    }

    private JSONObject delete(String url, JSONObject variables) {
        return delete(url, variables, null);
    }

    private JSONObject delete(String url, Object params) {
        return delete(url, null, params);
    }

    private JSONObject delete(String url, JSONObject variables, Object params) {
        return request(url, HttpMethod.DELETE, this.headers, variables, params, JSONObject.class).getBody();
    }
    ...
    private JSONObject post(String url, JSONObject variables) {
        return post(url, null, variables);
    }
    ...
    private JSONObject post(String url, JSONObject variables, Object params) {
        return request(url, HttpMethod.POST, this.headers, variables, params, JSONObject.class).getBody();
    }

    private <T> ResponseEntity<T> request(String url, HttpMethod method, HttpHeaders headers, JSONObject variables, Object params, Class<T> responseType) {
        if (headers == null) {
            headers = this.headers;
        } else {
            String authorization = this.headers.getFirst(HttpHeaders.AUTHORIZATION);
            if (Strings.isNotBlank(authorization)) {
                headers.set(HttpHeaders.AUTHORIZATION, authorization);
            }
        }
        return RestUtil.request(url, method, headers, variables, params, responseType);
    }
