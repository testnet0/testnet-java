...
import org.jeecg.common.constant.enums.WebHookTypeEnum;
...
//			}
    ...
    @Resource
    private WebhookMsgHandle webHookHandle;
    ...
    public void sendTemplateMessage(MessageDTO message) {
        ...
        if (MessageTypeEnum.WEBHOOK.getType().equals(messageType)) {
            sendWebHookeMessage("消息发送测试", message.getData(), message.getTemplateCode());
        } else {
    ...
    private String formatWebHookMessage(MessageConfig messageConfig, MessageDTO messageDTO) {
        WebHookTypeEnum webhookType = WebHookTypeEnum.fromType(messageConfig.getWebHookType());
        switch (webhookType) {
            case DINGTALK:
                JSONObject jsonObject = new JSONObject();
                jsonObject.put("msgtype", "markdown");
                JSONObject markdown = new JSONObject();
                markdown.put("title", messageDTO.getTitle());
                markdown.put("text", messageDTO.getContent());
                jsonObject.put("markdown", markdown);
                return jsonObject.toJSONString();
            case WEWORK:
                JSONObject jsonObject1 = new JSONObject();
                jsonObject1.put("msgtype", "markdown");
                JSONObject markdown1 = new JSONObject();
                markdown1.put("content", messageDTO.getContent());
                jsonObject1.put("markdown", markdown1);
            ...
            case FEISHU:
                ...
                zhCN.put("title", messageDTO.getTitle());
                JSONArray contentArray = new JSONArray();
                String[] paragraphs = messageDTO.getContent().split("\n");
                for (String paragraph : paragraphs) {
                    if (paragraph.trim().isEmpty()) continue;
                    JSONArray itemArray = new JSONArray();
                    JSONObject item = new JSONObject();
                    item.put("tag", "text");
                    item.put("text", paragraph.trim().replaceAll("#", ""));
                    itemArray.add(item);
                    contentArray.add(itemArray);
                }
    ...
    @Async
    @Override
    public void sendWebHookeMessage(String title, Map<String, Object> params, String templateCode) {
        messageConfigService.list().forEach(messageConfig -> {
            MessageDTO messageDTO = new MessageDTO();
            messageDTO.setTitle(title);
            messageDTO.setToUser(messageConfig.getId());
            messageDTO.setTemplateCode(templateCode);
            messageDTO.setData(params);
            messageDTO.setIsMarkdown(true);
            messageDTO.setType(MessageTypeEnum.WEBHOOK.getType());
            if (oConvertUtils.isNotEmpty(templateCode)) {
                SysMessageTemplate templateEntity = getTemplateEntity(templateCode);
                String content = templateEntity.getTemplateContent();
                if (oConvertUtils.isNotEmpty(content) && null != messageDTO.getData()) {
                    content = FreemarkerParseFactory.parseTemplateContent(content, messageDTO.getData(), true);
                }
                messageDTO.setContent(content);
            }
            messageDTO.setContent(formatWebHookMessage(messageConfig, messageDTO));
            messageDTO.setToUser(messageConfig.getWebHookUrl());
            webHookHandle.sendMessage(messageDTO);
        });
    }
